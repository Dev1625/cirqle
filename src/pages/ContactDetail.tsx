...
export default function ContactDetail() {
  ...
  const handleProcessReply = async (e: React.FormEvent) => {
     ...
     try {
        ...
        const response = await ai.models.generateContent({
           model: AI_FEATURES.replyParse.model,
           contents: prompt,
  ...
  const handleParseConversation = async (e: React.FormEvent) => {
     ...
     setIsProcessing(true);
     try {
        const ai = getGemini('conversationTags');
        ...
        const response = await ai.models.generateContent({
           model: AI_FEATURES.conversationTags.model,
           contents: prompt,
           config: { responseMimeType: "application/json" }
        });
        const parsed = JSON.parse(response.text || "{}");
  ...
  const generateDraft = async () => {
    ...
    try {
      ...
      const ai = getGemini('contactDraft');
...
Return the result in JSON format EXACTLY like this:
{
  ...
  "body": "The full email body"
      ...
      const response = await ai.models.generateContent({
        model: AI_FEATURES.contactDraft.model,
        contents: prompt,
        config: { responseMimeType: "application/json" }
      });
